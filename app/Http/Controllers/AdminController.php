...
use App\Models\Donation; // adapte au nom réel de ton modèle
use Illuminate\Support\Facades\DB;      // ✅ AJOUTER CECI
...
{
    $check = $this->checkAdmin(); if ($check !== true) return $check;
...
public function formationsIndex(Request $request)
{
    ...
    if ($orgFk) {
        $select[] = 'org.name as organizer_name';
                $select[] = 'org.email as organizer_email';   // ✅ ajoute l’email
...
public function formationsShow($id)
{
    ...
    $orgFk = null;
    foreach ($possibleFks as $fk) {
        if (\Illuminate\Support\Facades\Schema::hasColumn('formations', $fk)) { $orgFk = $fk; break; }
    }

    $q = \App\Models\Formation::query();
    if ($orgFk) {
    ...
    public function donationsHistory(Request $request)
{
    ...
    $select = ['donations.*'];
    if ($userFk) { $select[] = 'u.name as donor_name'; $select[] = 'u.email as donor_email'; }
    if ($campFk) { $select[] = 'e.title as campaign_name'; }
